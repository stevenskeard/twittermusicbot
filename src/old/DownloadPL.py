...
if len(url) == 34:
	call('youtube-dl --get-filename --ignore-errors --max-downloads 25 -o "%(id)s"' + ' ' + url + ' | tee logfile.txt', shell=True)

else:
	call('youtube-dl --get-filename --max-downloads 1 --ignore-errors -o "%(id)s"' + '  --default-search \"ytsearch\" \"' + url + '\" ' + ' | tee logfile.txt', shell=True)
